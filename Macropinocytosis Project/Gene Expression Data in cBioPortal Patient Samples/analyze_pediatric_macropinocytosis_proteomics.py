...
# ---- Global style tweaks (publication-ready with Garamond) ----
plt.rcParams.update({
...
# ---- Load txt files ----
pelp1 = pd.read_csv("Macropinocytosis Project/Gene Expression Data in cBioPortal Patient Samples/PELP1_expression_enrichment.txt", sep="\t")
snap23 = pd.read_csv("Macropinocytosis Project/Gene Expression Data in cBioPortal Patient Samples/SNAP23_expression_enrichment.txt", sep="\t")
...
def plot_gene_box(df, value_col, title):
    ...
    ax.set_ylabel("Protein level", fontsize=16, fontweight='bold')
    ax.set_xlabel("")
    ...
    ax.text(0.5, y + pad*1.2, stars, ha='center', fontsize=14, fontweight='bold')
...
def plot_gene_sem(df, raw_col, title):
    ...
    ax.set_ylabel("Normalized protein level", fontsize=16, fontweight='bold')
    ax.set_xlabel("")
    ax.set_title(title, fontsize=18, fontweight='bold', pad=12)
    ...
    ax.plot([0, 0, 1, 1], [y, y+pad, y+pad, y], lw=1.3, c='black')
    stars = "***" if p < 0.001 else "**" if p < 0.01 else "*" if p < 0.05 else "ns"
    ax.text(0.5, y + pad*0.9, stars, ha='center', fontsize=14, fontweight='bold')
